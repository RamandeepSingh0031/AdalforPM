    ...
    <nav style={{
      ...
      <div style={{ alignItems: 'center', boxSizing: 'border-box', display: 'flex', gap: '36px' }}>
        {location.pathname === '/' ? (
          ...
          <>
            {[
              { key: 'about', label: 'About', scrollToTop: true },
              { key: 'schedule', label: 'Schedule' },
              { key: 'speakers', label: 'Speakers' },
              { key: 'tickets', label: 'Tickets' },
              { key: 'faq', label: 'FAQ' }
            ].map((tab) => {
              const isActive = tab.key === 'about'
                ...
                <a
                  ...
                  onClick={(e) => {
                    if (tab.scrollToTop) {
                      ...
                      e.preventDefault();
                      navigate(`/conference#${tab.key}`);
                      setTimeout(() => {
                        const element = document.getElementById(tab.key);
                        if (element) element.scrollIntoView({ behavior: 'smooth' });
                      }, 100);
                    } else {
                      scrollToSection(e, tab.key);
                    }
                  }}
                  style={{ boxSizing: 'border-box', color: isActive ? '#CCC07F' : '#9A8E4E', display: 'inline-block', fontFamily: 'system-ui, sans-serif', fontSize: '15px', lineHeight: '18px', textDecoration: 'none', cursor: 'pointer', fontWeight: isActive ? 'bold' : 'normal', borderBottom: isActive ? '2px solid #CCC07F' : '2px solid transparent', paddingBottom: '4px' }}
                >
                  {tab.label}
                </a>
              );
